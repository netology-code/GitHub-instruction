...
Правилом хорошего тона для разработчика является не только умение грамотно писать код, но также и организовывать его хранение в репозитории. Для удобства доступа к файлам мы можем настроить систему оглавления в нашем репозитории по аналогии с блоком [навигации](#navigation) этой статьи. Поэтому нам потребуется добавить в наш репозиторий файл README.md, *который уже мог быть создан в вашем репозитории по умолчанию*, и познакомиться с основными возможностями **markdown**.
...
Для загрузки изображений в файл формата .md используется синтаксис, схожий с описанием внешней ссылки  **![альтернативный текст](ссылка на изображение)**:

<img src = "img/img20.png" width = 100%>
  
#
 
Важно, что путь до файла с кодом или изображением в вашем репозитории может быть указан через абсолютную или относительную ссылку. То есть в формате **https://адрес_файла** для указания абсолютной ссылки на файл опубликованный в сети или в формате **ДИРЕКТОРИЯ/ФАЙЛ.РАСШИРЕНИЕ** для указания относительного пути к файлу, находящемуся строго в текущем репозитории.
